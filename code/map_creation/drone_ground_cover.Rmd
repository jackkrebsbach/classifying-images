...
source("code/map_creation/vegetation_indicies.R")
library(sp)
library(rgdal)
library(raster)
library(rgeos)
...
First create a data frame, one quadrat per row, of average NDVI.
...
for (n in 34:83) {
  ...
  for (l in 0:3) {
    ...
    for (index in indices) {
      ave <- index(pixels)
      # Find the  average NDVI for each quadrat
...
Next create a data frame for the classified ground-based photographs, one quadrat per row, of pixel classes

## Ground Frame creation
```{r}
options(digits = 3)
x <- sprintf("./Quadrats_Processed/Quadrat_%03d/", seq(34, 83, 1))
...
for (n in 1:length(x)) {
  ...
  for (l in 0:3) {
    ...
    for (class in classes) {
      per <- subset(pixels, pixels == class)$Freq / total
      per[is.na(per)] <- 0
      row <- append(row, per)
    }
