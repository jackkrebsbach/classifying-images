...
polygon_data <- 
  ...
  right_join(layer_paths, by = "directory")
polygon_data
```
...
#   cluster_copy("extract_testing")

## Extract all pixels from polygons

We do this by layer so we can hold in memory the raster
```{r}
data <- tibble(layer_names = layer_names)

extract_testing <- function(polygon, image_path) {
  image <- raster::brick(image_path)
  exactextractr::exact_extract(img, polygon, include_cell = TRUE)
}

polygon_data

tic()
pixel_data <- polygon_data %>% 
  slice_sample(n = 10) %>%
  rowwise() %>%
  mutate(pixels = extract_testing(polys, layer_path)) %>%
  ungroup()
